...
path = 'analysis/'
% prefix = 'him-5 male SP new data 07162024'; % 8 cm
prefix = 'him-5 male SP new data'; 
% prefix = 'him_5 SP on LID SD calm down';
tmpprefix = prefix;

% pixel_per_micron = 1/25;
pixel_per_micron = 1;
...
polyorder = 3;
framesize = 95;
bad_inds = [2, 4, 53, 54, 79, 20, 100, 65, 13,...
...
for jj=1:Nsets
    ...
    for kk=1:ndata
        ...
        indx = 2*ii;
        indy = 2*ii+1;
        xx = data(:,indx)./pixel_per_micron;
        yy = data(:,indy)./pixel_per_micron;
        if(is_empty_data(xx))
            continue
        end
...
% for ii=37
    ...
    NN(ii)=length(tt);
    if(target_frame=='N') %  || target_frame=='-'
        % continue
        inds_not_find(jj)=ii;
        jj = jj+1;
        target_frame = NN(ii);
